...
export async function createSession(platform: App.Platform, token: string, userId: string) {
...
export async function validateSessionToken(platform: App.Platform, token: string) {
	const db = getDb(platform);
	const sessionId = encodeHexLowerCase(sha256(new TextEncoder().encode(token)));
